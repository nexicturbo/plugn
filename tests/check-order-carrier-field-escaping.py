...
assert_contains(renderer_content, "$url === null || $url === ''", RENDERER)
...
for view_path in ORDER_VIEWS:
    ...
    for field in [
        ...
        assert_contains(
            content,
            f"$model->{field} !== null && $model->{field} !== ''",
            view_path,
        )
        assert_not_contains(content, f"$model->{field} != null", view_path)
